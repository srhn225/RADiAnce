...
class EpsilonNetRAG(nn.Module):
    ...
    def forward(
        ...
        return eps_H, eps_X
class EpsilonNetRAGfull(nn.Module):

    def __init__(
            self,
            input_size,
            hidden_size,
            encoder_type='EPTragfull',
            opt={ 'n_layers': 3 }
        ):
        super().__init__()
    ...
    def forward(
        ...
        next_H, next_X = self.encoder(in_feat, X_noisy, block_ids, batch_ids, edges, edge_embed,prompt_feature=prompt_feature,prompt_mask=prompt_mask)
...
class FullDPMRAG(nn.Module):
    ...
    def sample(
        ...
        return traj 
class FullDPMRAGfull(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.eps_net = EpsilonNetRAGfull(latent_size, hidden_size, encoder_type, encoder_opt)
        self.num_steps = num_steps
        self.trans_x = construct_transition(trans_pos_type, num_steps, trans_pos_opt)
        self.trans_h = construct_transition(trans_seq_type, num_steps, trans_seq_opt)
    ...
    def forward(
        ...
        beta = self.trans_x.get_timestamp(t)[batch_ids]  # [N]
        eps_H_pred, eps_X_pred = self.eps_net(
            H_noisy, X_noisy, cond_embedding, edges, edge_types, generate_mask, batch_ids, beta,prompt_feature=prompt_feature,prompt_mask=prompt_mask,
        )
    ...
    def sample(
        ...
        for t in pbar(range(self.num_steps, 0, -1)):
            ...
            eps_H, eps_X = self.eps_net(
                H_t, X_t, cond_embedding, edges, edge_types, generate_mask, batch_ids, beta,prompt_feature,prompt_mask=prompt_mask
            )
...
class FullDPMCFG(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.eps_net = EpsilonNetCFG(latent_size, hidden_size, encoder_type, encoder_opt)
        self.num_steps = num_steps
        self.trans_x = construct_transition(trans_pos_type, num_steps, trans_pos_opt)
        self.trans_h = construct_transition(trans_seq_type, num_steps, trans_seq_opt)
...
class EpsilonNetRAGmask(nn.Module):

    def __init__(
            self,
            input_size,
            hidden_size,
            encoder_type='EPTragmask',
            opt={ 'n_layers': 3 }
        ):
        super().__init__()
    ...
    def forward(
        ...
        next_H, next_X = self.encoder(in_feat, X_noisy, block_ids, batch_ids, edges, edge_embed,prompt_feature=prompt_feature,generate_mask=generate_mask)
        ...
        return eps_H, eps_X
class FullDPMRAGmask(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.eps_net = EpsilonNetRAGmask(latent_size, hidden_size, encoder_type, encoder_opt)
        self.num_steps = num_steps
        self.trans_x = construct_transition(trans_pos_type, num_steps, trans_pos_opt)
        self.trans_h = construct_transition(trans_seq_type, num_steps, trans_seq_opt)
