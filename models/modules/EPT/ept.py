...
class XTransEncoderActrag(nn.Module):
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None):
        ...
        # return H, block_repr, graph_repr, V.reshape(Z.shape) + Z
        return H, V.reshape(Z.shape) + Z
@R.register('XTransEncoderActragmaskattn')
class XTransEncoderActragmask(nn.Module):
    def __init__(self, hidden_size, ffn_size, n_rbf, cutoff=7.0, z_requires_grad=False, 
                 edge_size=16, n_layers=3, n_head=4, pre_norm=False, use_edge_feat=False, sparse_k=3, local_mask=False, attn_bias=True,
                 efficient=False, vector_act='none', 
                 # use_ieconv=False, zero_conv=False, efficient_ieconv=False, ieconv_share_edge_feat=False
        ...
        self.encoder = Transformerragmask(
            d_hidden = hidden_size, d_ffn = ffn_size, n_heads = n_head, n_layers = n_layers,
            n_rbf = n_rbf, d_edge = edge_size, cutoff = cutoff, use_edge_feat = use_edge_feat, local_mask = local_mask, attn_bias = attn_bias,
            layer_norm = 'pre' if pre_norm else 'post', sparse_k = sparse_k, efficient = efficient,
            vector_act = vector_act, 
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None,generate_mask=None):
        H, V = self.encoder(H, Z, block_id, batch_id, edges, edge_attr, topo_edges, topo_edge_attr, attn_mask,prompt_feature=prompt_feature,generate_mask=generate_mask)
        block_repr = std_conserve_scatter_sum(H, block_id, dim=0)
        graph_repr = std_conserve_scatter_sum(block_repr, batch_id, dim=0)
        # return H, block_repr, graph_repr, V.reshape(Z.shape) + Z
        return H, V.reshape(Z.shape) + Z
@R.register('XTransEncoderActragfull')
class XTransEncoderActragfull(nn.Module):
    def __init__(self, hidden_size, ffn_size, n_rbf, cutoff=7.0, z_requires_grad=False, 
                 edge_size=16, n_layers=3, n_head=4, pre_norm=False, use_edge_feat=False, sparse_k=3, local_mask=False, attn_bias=True,
                 efficient=False, vector_act='none', 
                 # use_ieconv=False, zero_conv=False, efficient_ieconv=False, ieconv_share_edge_feat=False
        ...
        self.encoder = Transformerragfull(
            d_hidden = hidden_size, d_ffn = ffn_size, n_heads = n_head, n_layers = n_layers,
            n_rbf = n_rbf, d_edge = edge_size, cutoff = cutoff, use_edge_feat = use_edge_feat, local_mask = local_mask, attn_bias = attn_bias,
            layer_norm = 'pre' if pre_norm else 'post', sparse_k = sparse_k, efficient = efficient,
            vector_act = vector_act, 
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None,prompt_mask=None):
        H, V = self.encoder(H, Z, block_id, batch_id, edges, edge_attr, topo_edges, topo_edge_attr, attn_mask,prompt_feature=prompt_feature,prompt_mask=prompt_mask)
        block_repr = std_conserve_scatter_sum(H, block_id, dim=0)
        graph_repr = std_conserve_scatter_sum(block_repr, batch_id, dim=0)
        # return H, block_repr, graph_repr, V.reshape(Z.shape) + Z
        return H, V.reshape(Z.shape) + Z
...
class Transformerragmask(nn.Module):
    '''Equivariant Adaptive Block Transformer'''
    ...
    def __init__(
        ...
        for i in range(0, n_layers):
            self.add_module(f'layer_{i}', EPTLayerragmask(
                d_hidden, d_ffn, n_heads, i, act_fn, layer_norm, residual, self.efficient, vector_act, attn_bias
            ))#n_layers: 6
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None,generate_mask=None):
        ...
        batch_to_nodes = batch_id[block_id]
        H_batch, H_mask = graph_to_batch_nx(H, batch_to_nodes, mask_is_pad=False, factor_req=8)
        generate_mask_batch, generate_mask_mask = graph_to_batch_nx(generate_mask.long(), batch_to_nodes, mask_is_pad=False, factor_req=8)#1 for generate
        bs, max_n = H_batch.shape[0], H_batch.shape[1]
        V_batch = torch.zeros((bs, max_n, *V.shape[1:]), dtype=V.dtype, device=V.device)
        V_batch[H_mask] = V
        Z_batch = torch.zeros((bs, max_n, *Z.shape[1:]), dtype=Z.dtype, device=Z.device)
        ...
        for i in range(self.n_layers):
            H_batch, V_batch = self._modules[f'layer_{i}'](
                H_batch, V_batch, cached_info,prompt_feature,generate_mask=generate_mask_batch
            )
...
class Transformerragincontext(nn.Module):
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None):
        ...
        _, V_graph = self.final_v(H_graph, V_graph)
        return H_graph, V_graph
class Transformerragfull(nn.Module):
    '''Equivariant Adaptive Block Transformer'''
    ...
    def __init__(
        ...
        for i in range(0, n_layers):
            self.add_module(f'layer_{i}', EPTLayerragfull(
                d_hidden, d_ffn, n_heads, i, act_fn, layer_norm, residual, self.efficient, vector_act, attn_bias
            ))#n_layers: 6
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, topo_edges=None, topo_edge_attr=None, attn_mask=None,prompt_feature=None,prompt_mask=None):
        ...
        for i in range(self.n_layers):
            H_batch, V_batch = self._modules[f'layer_{i}'](
                H_batch, V_batch, cached_info,prompt_feature,prompt_mask
            )
...
class EPTLayerragfull(nn.Module):
    def __init__(
        self,
        d_hidden,
        d_ffn,
        ...
        super(EPTLayerragfull, self).__init__()
        ...
        if use_cross_attn:
            self.cross_attn_layer = SubLayerWrapperH(
                CrossAttentionLayerfull(d_hidden, n_heads),  
                d_hidden, layer_norm, residual
            )
    ...
    def forward(self, H, V, cached_info=None, prompt_feature=None,prompt_mask=None):
        ...
        if self.use_cross_attn and prompt_feature is not None:
            ...
            H = self.cross_attn_layer(H, cached_info=cached_info,prompt_feature=prompt_feature,prompt_mask=prompt_mask)
            # print(H.shape)
        else:
...
class CrossAttentionLayerfull(nn.Module):
    ...
    def forward(self, H, cached_info, prompt_feature,prompt_mask):
        res, attn_weight = self.attn(H, prompt_feature, prompt_feature,key_padding_mask=prompt_mask)
        # torch.set_printoptions(profile="full")
        # # # print(attn_weight.mean(dim=1))
        # print(attn_weight)
        return self.scaler_o(res)
...
class EPTLayerragmask(nn.Module):
    def __init__(
        self,
        d_hidden,
        d_ffn,
        ...
        super(EPTLayerragmask, self).__init__()
        ...
        if use_cross_attn:
            self.cross_attn_layer = SubLayerWrapperH(
                CrossAttentionLayer(d_hidden, n_heads),  
                d_hidden, layer_norm, residual=False
            )
    ...
    def forward(self, H, V, cached_info=None, prompt_feature=None, generate_mask=None):
        """
        H: [B, N, D]
        generate_mask: [B, N], dtype=torch.bool or 0/1
        """
        ...
        if self.use_cross_attn and prompt_feature is not None:
            ...
            masked_H = H * generate_mask.unsqueeze(-1)  # [B, N, D]



            delta_H = self.cross_attn_layer(masked_H, cached_info=cached_info, prompt_feature=prompt_feature)



            H = H + delta_H * generate_mask.unsqueeze(-1)
        else:
            print("Warning: No cross-attn")  # no cross-attn
            exit(0)
            pass  # no cross-attn
