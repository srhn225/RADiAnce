...
def _is_in_the_range(res_id, selected_residues):
    '''
        Args:
            res_id: e.g. ('A', (19, ''))
            selected_residues: list of res_id
    '''
    selected = {}
    for chain_id, sel_res_id in selected_residues:
        selected[(chain_id, tuple(sel_res_id))] = True
    insert_code = res_id[1][-1].strip()
    res_id = (res_id[0], (res_id[1][0], insert_code))
    return res_id in selected_residues


class ForceFieldMinimizer(object):

    def __init__(self, stiffness=10.0, max_iterations=0, tolerance=2.39, platform='CUDA'):
        super().__init__()
    ...
    def _minimize(self, pdb_str):
        ...
        if self.platform == 'CPU': platform.setPropertyDefaultValue('Threads', '1') # only use one cpu per relaxation
